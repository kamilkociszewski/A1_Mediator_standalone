...
int main(int argc, char* argv[])
{
    ...
    // Start A1
    ////////////
    int policyTypeId = 40008;
    int policyInstanceId = 48;
    int threshold = 10;
    srand(time(NULL)); // Seed random number generation with current time
    // Initialize CURL globally
    curl_global_init(CURL_GLOBAL_DEFAULT);
    // Fetch and print the list of policy types
    printf("\nFetch policy types\n");
    fetchPolicyTypes();
    // Call the function to create or update policy type
    printf("Create or update policy type ID %d\n", policyTypeId);
    createOrUpdatePolicyType(policyTypeId);
    printf("Fetch policy types\n");
    fetchPolicyTypes();
    printf("Fetch details for policy type %d\n", policyTypeId);
    fetchPolicyTypeDetails(policyTypeId);
    printf("Create or update policy instance %d under policy type %d with threshold %d\n", policyInstanceId, policyTypeId, threshold);
    updatePolicyThreshold(policyTypeId, policyInstanceId, threshold);
    printf("Fetch all policy instances under policy type %d\n", policyTypeId);
    fetchAllPolicies(policyTypeId);
    printf("Check status of policy instance %d\n", policyInstanceId);
    checkPolicyStatus(policyTypeId, policyInstanceId);
    threshold = (rand() % 21) + 10;
    printf("Update policy instance %d under policy type %d with threshold %d\n", policyInstanceId, policyTypeId, threshold);
    updatePolicyThreshold(policyTypeId, policyInstanceId, threshold);
    printf("Check status of policy instance %d after update\n", policyInstanceId);
    checkPolicyStatus(policyTypeId, policyInstanceId);
